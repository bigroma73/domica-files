...
# Coded by vlamo (c) 2011, moded by bigroma (06.07.2011)
#
...
class ServiceName2(Converter, object):
	...
	def getText(self):
		...
		if self.type == self.NAME:
			...
			return "%0.4X" % info.getInfo(iServiceInformation.sSID)
		elif self.type == self.XAPID:
			return "%0.4X" % info.getInfo(iServiceInformation.sAudioPID)
		elif self.type == self.XVPID:
			try:
				return "%0.4X" % info.getInfo(iServiceInformation.sVideoPID)
			except:
				return ""
